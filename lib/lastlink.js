...
async function processWebhook(rawPayload, headers) {
    ...
    if (status === 'active' || status === 'trialing') {
      await c.query(
        `UPDATE organizations
         SET plan_code = $1, max_clients = $2, max_collaborators = $3, ia_quota_limit = $4, updated_at = now()
         WHERE id = $5`,
        [plan.code, plan.max_clients, plan.max_collaborators, plan.ia_quota_limit, orgId]
      );
    } else if (status === 'canceled' || status === 'past_due') {
      const free = getPlan('free');
      await c.query(
        `UPDATE organizations
         SET plan_code = 'free', max_clients = $1, max_collaborators = $2, ia_quota_limit = $3, updated_at = now()
         WHERE id = $4`,
        [free.max_clients, free.max_collaborators, free.ia_quota_limit, orgId]
      );
    }
  });
  ...
  return { ok: true, info, plan: plan.code, status, organization_id: orgId };
}
