...
// PDF do dashboard / visao geral da agencia.
// data = { orgName, stats, overview } (mesmos campos de GET /api/dashboard/stats).
function dashboardPdf({ orgName, stats, overview }) {
  const doc = new PDFDocument({ size: 'A4', margin: 40, info: {
    Title: `Dashboard - ${orgName || 'SocialFlow'}`,
    Author: 'SocialFlow',
  }});

  header(doc, {
    title: 'Dashboard',
    subtitle: `${orgName || ''}  ·  ${new Date().toLocaleDateString('pt-BR')}`,
  });

  const line = (label, value) => {
    doc.fillColor(BRAND.text3).font('Helvetica').fontSize(11).text(String(label), { continued: true });
    doc.fillColor(BRAND.text1).font('Helvetica-Bold').text(`  ${value}`);
  };

  const s = stats || {};
  sectionTitle(doc, 'Resumo');
  line('Clientes ativos', s.clients ?? 0);
  line('Personas cadastradas', s.personas ?? 0);
  line('Aguardando aprovacao', s.pending ?? 0);
  line('Aprovados no mes', s.approved_month ?? 0);
  line('Rascunhos', s.drafts ?? 0);

  const ov = overview || {};
  const st = ov.by_status || {};
  sectionTitle(doc, 'Conteudo por status');
  line('Aprovado', st.approved ?? 0);
  line('Pendente', st.pending_approval ?? 0);
  line('Rascunho', st.draft ?? 0);

  const fmt = ov.by_format || {};
  sectionTitle(doc, 'Conteudo por tipo');
  line('Reels', fmt.reel ?? 0);
  line('Carrossel', fmt.carrossel ?? 0);
  line('Post estatico', fmt.post ?? 0);
  line('Stories', fmt.story ?? 0);

  const raw = ov.raw_material || {};
  sectionTitle(doc, 'Material bruto');
  line('Clientes com pasta no Drive', `${raw.with_drive ?? 0} / ${raw.total ?? 0}`);
...
module.exports = { scriptPdf, carouselPdf, dashboardPdf, safeFilename };
