...
function carouselPdf(carousel) {
  ...
  // Se houver imagem de capa (gerada por IA), desenha como banner no topo
  // com a faixa da marca por cima pra manter o titulo legivel.
  if (carousel.cover_buffer) {
    let drew = false;
    try {
      doc.image(carousel.cover_buffer, 0, 0, { width: doc.page.width, height: 200, cover: [doc.page.width, 200] });
      drew = true;
    } catch {}
    if (drew) {
      // faixa semi-transparente + titulo branco sobre a imagem
      doc.save();
      doc.rect(0, 140, doc.page.width, 60).fillOpacity(0.55).fill('#000000');
      doc.restore();
      doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20)
        .text(String(carousel.title || 'Carrossel').slice(0, 80), 40, 156, { width: doc.page.width - 80 });
      doc.fillOpacity(1);
      doc.y = 220;
      doc.x = 40;
    } else {
